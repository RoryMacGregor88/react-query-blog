...
const getUser = rest.get('*/api/users/:id', (req, res, ctx) =>
  res(ctx.status(200), ctx.json(getMockUsers().find(u => u.id === Number(req.params.id)))),
);

const getUserPosts = rest.get('*/api/users/:id/posts', (req, res, ctx) =>
  res(ctx.status(200), ctx.json(getMockPosts().filter(p => p.authorId === Number(req.params.id)))),
);
